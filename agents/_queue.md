...
source: Q-Day Observatory comparison, Phase 3 review
    ...
    - **`qualifier` is a grouping key, not a description, and it is short.**
      Two measurements are compared only when their qualifiers match exactly,
      so reuse a phrase already on the board wherever the condition is the
      same. A previous attempt at this job wrote 94-character qualifiers
      describing the apparatus and lost the whole run; the detail belongs in
      `note`, which is five times longer and which nothing groups on.
