...
- Anything naming Ofgem, a live consultation or a regulatory position escalates
  rather than merging — see §7a for the full auto-merge model
...
## 7a. Autonomy: publish first, veto after

Decided 7 Aug 2026, replacing review-before-merge. The reasoning is workload
arithmetic: five galaxies on the original model is roughly six pull requests a
week and three hours of review. That fails within a month, and **a review queue
that becomes wallpaper is worse than no agents**, because the board keeps
publishing regardless.

So: agents merge and publish. You veto afterwards.

### The exchange being made

Review-before-merge trades *your time* for *certainty*. Publish-first trades
*certainty* for *visible provenance*. The second only works if the provenance is
genuinely unmissable, so that is where the effort goes.

### Every item carries its own status, always

No item on the board is ever unlabelled. Three states:

| State | Meaning | Shown as |
| --- | --- | --- |
| `reviewed` | A human has read it | plain, with the review date |
| `agent-merged` | Published by an agent, not yet read | **labelled everywhere it appears** |
| `vetoed` | Reverted by a human | removed, and logged |

An agent writes `agent-merged` and **cannot write `reviewed`**. There is no path
by which agent output quietly acquires the appearance of human judgement. That
single rule carries most of the safety.

Displayed in full:

> **Below-threshold surface code memory** · E4 · demonstrated
> *Agent-merged 12 Aug 2026 — not yet reviewed*
> Last human review of this galaxy: 7 Aug 2026

The two dates do the work between them. A recent agent merge against an old
review date is visibly unchecked, and the reader can weigh that themselves.

### The veto

Weekly, an issue lists everything merged since the last one, with links.

- **Confirm** — tick the item; state moves to `reviewed`, dated
- **Veto** — GitHub's own Revert button on the merge commit. One click, and it
  is logged in the item's history
- **Ignore** — it stays `agent-merged` and keeps saying so, indefinitely

Ignoring is a legitimate outcome. It is not a backlog and does not accumulate as
debt, because the item is already telling the reader it has not been checked.

### Review debt is displayed, not hidden

Per galaxy, on the board:

> **Quantum** · 118 items · 41 agent-merged, unreviewed · last human review 9 weeks ago

If you fall behind, the site says so. That is strictly better than presenting
stale content as current, and it is the same honesty principle as the hollow
unsourced bodies.

### What still does not auto-merge

Short list, and I would argue for keeping it short rather than empty.

1. **Anything naming Ofgem, a live consultation, or a regulatory position.**
   Not a technical risk — a professional one. This is the category where an
   unreviewed sentence under your name causes real difficulty, and no
   provenance label fixes it.
2. **Deletion or archiving of an existing published item.** Additions are
   reversible by reverting; a quiet removal is how something disappears without
   anyone noticing. Agents propose removals, humans action them.
3. **Any edit to a field in an item's `locked` array.** Unchanged.

Everything else merges: new items, evidence changes in either direction,
readiness moves, Q-Day impact scores, E5 promotions, and Q-Day forecast changes
— all labelled, all revertible.

### Why Q-Day forecast changes can now auto-merge

Consistent with the decision of 7 Aug: an agent may move the estimate, it is
stamped `agent-estimate`, it cannot clear that stamp, and it displays the last
human review date beside it. The guardrails in §4 still apply — one axis at a
time, two-year cap, evidence required, never on a roadmap.

A labelled agent estimate that says so is more honest than a stale human one
that does not.

### What makes this safe enough to be worth it

- Provenance labelling is enforced in the renderer. **A missing state label
  fails the build**, so an unlabelled item cannot reach the site
- Every deploy is immutable and versioned; revert is one action
- Scope gates are unchanged — agents still cannot touch the renderer, the
  schema, the scripts or the workflows
- Downgrades are as automatic as upgrades. An agent making the board *less*
  confident cannot embarrass anyone, and must never be harder than the reverse

### The honest residual risk

An agent will eventually publish something wrong under your name, and it will
sit there until you look. The mitigation is not that this cannot happen — it is
that the item says plainly it has not been checked, and that removing it takes
one click.

If that trade ever stops feeling right, the fix is **cadence, not filtering**.
Fewer runs, not more gates.
...
## 11a. The bundling ceiling

Found 8 Aug 2026, when four sourced items pushed the entry chunk past its
budget.

Content is bundled into JavaScript at build time. That is right for 56 items
and wrong for 500: the bundle grows linearly with the board, and every visitor
downloads every item before seeing anything.

**Now:** content is split into its own chunk with its own budget. Application
code and data are measured separately, so the app does not appear to bloat every
time an agent does its job.

**At roughly 200 items, or 220 KB gzipped of content**, the fix is to emit
content as a JSON file fetched at runtime rather than bundled. The budget gate
says so when it fails, rather than tempting whoever hits it to raise the number.

Do not raise the ceiling instead. That is the failure this note exists to
prevent.
